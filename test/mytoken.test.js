...
contract("My test", async function([user1,user2,user3]){
    ...
    describe( "시나리오 1", function(){
        ...
        it("배포한 컨트랙트의 데시멀은 '18' 입니까?", async function() {
            const decimal = await this.contract.decimals()
            assert.equal(decimal, 18, 'decimal이 일치하지 않습니다.')
        })
        ...
        it("0X0 주소로 전송하면 트랜잭션은 실패되어야 한다", async function(){
            try{
                await this.contract.transfer("0x0000000000000000000000000000000000000000", 100,{from: user1})
            } catch(e){
                return
            }
            assert(false,'트랜잭션은 실패되지 않음')
        })
    })
